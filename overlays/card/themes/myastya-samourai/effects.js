/* ============================================================
   Card — Effets MYASTYA-SAMOURAI
   Pétales de cerisier (canvas) + étincelles + fleur + son.
   Chargé/déchargé automatiquement par main.js (loadThemeEffects).
...
(function () {
  ...
  // Décharge une éventuelle instance précédente (sécurité si rechargé)
  ...
  var SFX = (function () {
    ...
    // Chargement optionnel des fichiers déclarés dans sounds.json
    function loadFiles() {
  ...
  /* ========================================================
     3) FLEUR de cerisier décorative (SVG)
     ======================================================== */
  var flowerEls = [];
  ...
  // PER-OVERLAY : fleur perchée sur le coin haut-gauche de la card
  var FLOWERS = [
    { parent: '.card', style: { position: 'absolute', left: '-16px', top: '-20px', width: '58px', height: '58px', zIndex: '5' } }
  ];
  ...
  /* ========================================================
     4) HOOKS — branchés sur les animations/transitions existantes
        (aucune modification du JS coeur de l'overlay)
     ======================================================== */
  function onAnim(e) {
    switch (e.animationName) {
      case 'cardIn':    SFX.play('appear'); burstAtEl(document.getElementById('card'), 14, 'center'); break;
      case 'mys-flash': SFX.play('tick'); break;
    }
  }
  function onTransStart(e) {
    if (e.target && e.target.id === 'xp-fill' && e.propertyName === 'transform') SFX.play('fill');
  }
  ...
  // Débloque l'audio sur la 1re interaction (utile en prévisualisation navigateur)
  ...
  function destroy() {
    ...
    document.removeEventListener('transitionstart', onTransStart, true);

/* ============================================================
   CheckIn — Effets MYASTYA-SAMOURAI v2
   Déclenchement UNIQUEMENT sur les animations existantes.
   - ci-stamp    → pétale qui se pose depuis le haut
   - ci-box-fill → petit pétale qui s'épanouit sur la case
   - ci-pulse    → burst d'étincelles (carte complète)
...
(function () {
  ...
  /* ========================================================
     2) CANVAS — étincelles en burst uniquement (carte complète)
     ======================================================== */
  ...
  function Spark(x, y) {
    var a = Math.random() * Math.PI * 2, spd = 1.6 + Math.random() * 3.8;
    this.x = x + (Math.random() - 0.5) * 12;
    this.y = y + (Math.random() - 0.5) * 12;
    this.vx = Math.cos(a) * spd; this.vy = Math.sin(a) * spd - 0.6;
    this.life = 0; this.maxLife = 18 + Math.random() * 22;
  ...
  Spark.prototype.update = function () { this.life++; this.x += this.vx; this.y += this.vy; this.vx *= 0.91; this.vy = this.vy * 0.91 + 0.07; this.rot += 0.05; return this.life < this.maxLife; };
  ...
  function burstAtEl(el, n) {
    ...
    burst(r.left + r.width / 2, r.top + r.height / 2, n || 20);
  }
  function startBurst() {
  ...
  /* ========================================================
     3) HELPERS DOM — pétale SVG réutilisable
     ======================================================== */
  var uid = 0;
  var domEls = [];
  ...
  function spawnEl(cls, size, left, top, timeout) {
    var el = document.createElement('div');
    el.className = cls;
    el.innerHTML = petalSVG(size);
    el.style.left = left + 'px';
    el.style.top  = top  + 'px';
    document.body.appendChild(el);
    domEls.push(el);
    function rm() { if (el.parentNode) el.parentNode.removeChild(el); var i = domEls.indexOf(el); if (i >= 0) domEls.splice(i, 1); }
    el.addEventListener('animationend', rm, { once: true });
    setTimeout(rm, timeout || 3000);
    return el;
  }

  /* Gros pétale qui se pose depuis le haut — apparition du check-in */
  function spawnSettle() {
    var c = document.getElementById('checkin-container');
    if (!c) return;
    var r = c.getBoundingClientRect();
    // centré horizontalement, part du haut du container
    spawnEl('mys-settle', 70, r.left + r.width / 2 - 35, r.top - 5, 2800);
  }

  /* Petit pétale qui s'épanouit au centre de la case cochée */
  function spawnBoxBloom(box) {
    if (!box) return;
    var r = box.getBoundingClientRect();
    spawnEl('mys-box-bloom', 28, r.left + r.width / 2 - 14, r.top + r.height / 2 - 14, 700);
  }

  /* ========================================================
     4) FLEUR DE CERISIER DÉCORATIVE (ancrée sur le container)
     ======================================================== */
  var flowerEls = [];
  ...
  function buildFlower() {
    var parent = document.getElementById('checkin-container');
    if (!parent || flowerEls.length) return;
    var f = document.createElement('div');
    f.className = 'mys-flower';
    f.innerHTML = makeFlowerSvg('ci0');
    f.style.position = 'absolute';
    f.style.left = '-18px'; f.style.top = '-22px';
    f.style.width = '54px'; f.style.height = '54px';
  ...
  /* ========================================================
     5) HOOKS
     ======================================================== */
  // ci-stamp : animationstart de l'icône ✓ (une seule fois par affichage)
  var _lastStamp = 0;
  function onAnim(e) {
    switch (e.animationName) {
      case 'ci-stamp':
        // throttle : 1 settle max toutes les 300 ms (re-show rapide)
        if (Date.now() - _lastStamp < 300) break;
        _lastStamp = Date.now();
        SFX.play('appear');
        spawnSettle();
        break;
      ...
      case 'ci-box-fill':
        SFX.play('petal', { volume: 0.75 });
        spawnBoxBloom(e.target);
        break;

      case 'ci-pulse':
        // carte complète — une seule fois (animation infinite mais animationstart ne fire qu'1 fois)
        SFX.play('complete');
        burstAtEl(document.getElementById('checkin-container'), 32);
        break;
    }
  }
  ...
  console.log(LOG, 'v2 — pétale settle + bloom + burst on-demand + son. Aucun effet permanent.', SFX.isMuted() ? '(muet)' : '');
})();
